...
public class DescQuimicosMod extends UtilitarioMod<DescripcionQuimico> implements Serializable {
	...
	public List<DescripcionQuimico> todos() {
		...
		try {
			ResultSet rst = this.getCn().conectar()
					...
					.executeQuery();
			while (rst.next()) {
				this.getLis().add(new DescripcionQuimico(rst.getInt("usu_id_UltMod"), rst.getDate("fecha_id"),
	...
	public DescripcionQuimico buscado(int id) {
		try {
			...
			ResultSet rst = pst.executeQuery();
			if (rst.next()) {
				this.setObj(new DescripcionQuimico(rst.getInt("usu_id_UltMod"), rst.getDate("fecha_id"),
						rst.getDate("fecha_mod"), rst.getInt("desq_id"), rst.getInt("qui_id"),
						rst.getString("desq_desc"), rst.getString("desq_estm"), rst.getString("desq_color"),
						rst.getString("desq_umedida"), rst.getString("desq_infla")));
			}
...
//PETICIONES DE ESCRITURA
	...
	public boolean guardar(DescripcionQuimico nuevo) throws Exception {
		...
		try {
			...
			pst.setString(1, nuevo.getDesq_desc());
			pst.setString(2, nuevo.getDesq_estm());
			pst.setString(3, nuevo.getDesq_color());
			pst.setString(4, nuevo.getDesq_umedida());
			pst.setString(6, nuevo.getDesq_infla());
	...
	public boolean actualizar(DescripcionQuimico actual) throws Exception {
		...
		try {
			...
			pst.setString(1, actual.getDesq_desc());
			pst.setString(2, actual.getDesq_estm());
			pst.setString(3, actual.getDesq_color());
			pst.setString(4, actual.getDesq_umedida());
			pst.setString(5, actual.getDesq_infla());
